...
class Article:
    ...
    @staticmethod
    def loadArticles(names):
        index = defaultdict(Article)
    ...
    @staticmethod
    def findChains(articles):
        leafNodes = list(filter(lambda a: len(a.children) == 0, articles))
        for article in leafNodes:
            article.chain = article.findChain([])
            depth = len(article.chain)
            for a in article.chain:
                a.maxDepth = max(a.maxDepth, depth)

    def findChain(self, chain):
        if self in chain:
            chain[-1].terminal += 1
            return chain
        chain.append(self)
    ...
    def printArticles(articles):
        terminalArticles = list(filter(lambda a: a.terminal > 0, articles))
        terminalArticles.sort(reverse=True, key=lambda a: a.terminal)
        for article in terminalArticles:
            article.printArticle(0)

    def printArticle(self, depth):
        if not self.printed:
            print("·" * depth + self.name)
            self.printed = True
            self.children.sort(reverse=True, key=lambda a: a.maxDepth)
            for article in self.children:
                article.printArticle(depth + 1)

articles = Article.loadArticles(articleNames)
