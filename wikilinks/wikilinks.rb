...
class Article
  ...
  def self.loadArticles(names)
    index = Hash.new { |hash,name| hash[name] = Article.new(name) }
    names.each do |name, link|
      article = index[name]
      parent = index[link]
  ...
  def self.printArticles(articles)
    articles
