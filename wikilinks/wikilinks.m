...
@property NSMutableArray *chain;
@property BOOL terminal;
@property int maxDepth;
@property BOOL printed;

+ (Article *) articleWithName: (NSString *) name;
+ (Article *) getArticle: (NSString *) name;
+ (NSArray *) loadArticles: (NSDictionary *) articleNames;
+ (void) findChains: (NSArray *) articles;
- (NSMutableArray *) findChain: (NSMutableArray *) chain;
...
- (NSArray *) sortBy:(NSString *)key reverse:(BOOL)reverse;

@end

@implementation Article

static NSMutableDictionary *articleIndex;

+ (void) initialize {
  articleIndex = [NSMutableDictionary dictionary];
}

+ (Article *) articleWithName: (NSString *) name {
  Article *article = [[Article alloc] init];
  article.name = name;
  article.children = [NSMutableArray array];
  article.terminal = NO;
  article.maxDepth = 0;
  article.printed = NO;
  return article;
...
+ (Article *) getArticle: (NSString *) name {
  Article *article = articleIndex[name];
  if (article == nil) {
    article = [Article articleWithName: name];
...
+ (NSArray *) loadArticles: (NSDictionary *) articleNames {
    for (NSString *name in articleNames) {
      NSString *link = [articleNames objectForKey: name];
      Article *article = [Article getArticle: name];
      Article *parent = [Article getArticle: link];
      article.parent = parent;
      [parent.children addObject: article];
    }

    return [articleIndex allValues];
}

+ (void) findChains: (NSArray *) articles {
  [[articles filter: ^BOOL(Article *article) {
    return [article.children count] == 0;
  }] each: ^(Article *article) {
    article.chain = [article findChain: [NSMutableArray array]];
    int depth = [article.chain count];
    [article.chain each: ^(Article *a) {
      if (depth > a.maxDepth) {
        a.maxDepth = depth;
      }
    }];
...
- (NSMutableArray *) findChain: (NSMutableArray *) chain {
  if ([chain containsObject: self]) {
    ((Article *)[chain lastObject]).terminal += 1;
    return chain;
  }
  [chain addObject: self];
  return [self.parent findChain: chain];
}
...
- (NSString *) times: (int) count {
  if (count <= 0) {
    ...
    return [self stringByAppendingString: [self times: count - 1]];
  }
}
