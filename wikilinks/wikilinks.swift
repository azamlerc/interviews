...
class Article: Equatable {
  ...
  static func == (a: Article, b: Article) -> Bool {
    return a.name == b.name
  }

  static func loadArticles(names: [String:String]) -> [Article] {
    var index = [String:Article]()
    for (name, link) in names {
      let article = index[name, default: Article(name: name)]
      let parent = index[link, default: Article(name: link)]
      index[name] = article
      index[link] = parent
  ...
  func findChain(chain: [Article]) -> [Article] {
    guard !chain.contains(self) else {
      chain.last!.terminal += 1
      return chain
    }
    var newChain = chain
    newChain.append(self)
    return parent!.findChain(chain: newChain)
  }

  static func printArticles(articles: [Article]) {
    articles
      .filter { $0.terminal > 0 }
  ...
  func printArticle(depth: Int) {
    if !printed {
      ...
      children
        ...
        .forEach { $0.printArticle(depth: depth + 1) }
    }
  }
}

let articles = Article.loadArticles(names: articleNames)
Article.findChains(articles: articles)
Article.printArticles(articles: articles)
