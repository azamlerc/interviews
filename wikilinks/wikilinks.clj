...
; 3. Call (printArticle article 0) on each unique root node, taking care to avoid
;    printing any article more than once or getting stuck in an infinite loop.
...
(defn contains [array value] (some (partial = value) array))
(defn difference [a b] (vec (clojure.set/difference (set a) (set b))))
(defn indent [depth] (apply str (repeat depth "  ")))

(defn addToChain [article chain] (if (contains chain article) chain
  (addToChain (get articleIndex article) (conj chain article))))
(defn getChain [article] (addToChain article (list)))
(defn addToChildren [index article]
  (update index (get articleIndex article) conj article))

(def articles (keys articleIndex))
(def parentArticles (vals articleIndex))
(def featuredArticles (sort (difference articles parentArticles)))
(def chains (map getChain featuredArticles))
(def rootArticles (remove #(= % "Object") (distinct (map first chains))))
(def children (reduce addToChildren {} articles))

(defn printArticle [article depth]
  (println (indent depth) article)
  (doseq [article (get children article)]
    (if (not (contains rootArticles article))
      (printArticle article (inc depth)))))

(doseq [article rootArticles] (printArticle article 0))