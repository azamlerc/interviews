...
function getArticle(index: object, name: string): Article {
  let article: Article = index[name] || {name, children: [], terminal: 0, maxDepth: 0};
  index[name] = article;
  return article;
}
...
function findChain(article: Article, chain: Article[]): Article[] {
...
function findChains(articles: Article[]) {
  articles
...
function printArticle(article: Article, depth: number = 0) {
