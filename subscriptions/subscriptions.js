...
while (date.getFullYear() == 2022) {
  for (let user of users) {
    if (user.start == date.getTime()) {
      total++;
    }
    if (user.end == date.getTime()) {
      total--;
    }   
  }
...
// Estension: remove to the check for !user.start to allow
// users to re-subscribe. How does that affect your solution?